...
/*
Warp-code-major factor LUT layout:

  factor_lut[h, stage, code, lane]
  shape [1,H,4,16,32]

Flat index:
  ((((h * 4 + stage) * 16 + code) * 32) + lane)

The current factor-LUT kernel uses [h, coord, code].
This variant keeps the same numerical factor values, but remaps physical
storage so each (stage, code) tile contains a contiguous 32-lane segment.
*/
__global__ void turboquant_full_4bit_lane_word_factor_lut_warp_code_major_lane_nibble_qjl128_logits_b1q1_d128_kernel(
    const float* __restrict__ warp_code_major_factor_lut,
    const uint8_t* __restrict__ lane_word_scalar_codes,
    ...
    for (int stage = 0; stage < STAGES; ++stage) {
        const int coord = lane + stage * LANES;

        const uint8_t scalar_code =
            static_cast<uint8_t>((lane_word >> (stage * 4)) & 0x0Fu);
        const int64_t lut_idx =
            (((static_cast<int64_t>(h) * STAGES + stage) * CODES + scalar_code) * LANES)
            + lane;
        scalar_acc += warp_code_major_factor_lut[lut_idx];
...
void validate_warp_code_major_factor_lut(torch::Tensor factor_lut) {
    TORCH_CHECK(factor_lut.is_cuda(), "warp_code_major_factor_lut must be CUDA");
    TORCH_CHECK(factor_lut.dtype() == torch::kFloat32, "warp_code_major_factor_lut must be float32");
    TORCH_CHECK(
        factor_lut.dim() == 5 &&
        factor_lut.size(0) == 1 &&
        factor_lut.size(2) == STAGES &&
        factor_lut.size(3) == CODES &&
        factor_lut.size(4) == LANES,
        "warp_code_major_factor_lut must be [1,H,4,16,32]"
    );
}
...
torch::Tensor turboquant_full_4bit_lane_word_factor_lut_warp_code_major_lane_nibble_qjl128_logits_b1q1_d128_cuda(
    torch::Tensor warp_code_major_factor_lut,
    ...
    validate_warp_code_major_factor_lut(warp_code_major_factor_lut);
    ...
    TORCH_CHECK(warp_code_major_factor_lut.size(1) == lane_word_scalar_codes.size(1), "head mismatch");
    TORCH_CHECK(warp_code_major_factor_lut.size(1) == qjl_projected_queries.size(1), "head mismatch");
    TORCH_CHECK(warp_code_major_factor_lut.size(1) == lane_nibble_qjl_signs.size(1), "head mismatch");
    TORCH_CHECK(warp_code_major_factor_lut.size(1) == residual_norms.size(1), "head mismatch");
    ...
    const int H = static_cast<int>(warp_code_major_factor_lut.size(1));
    const int T = static_cast<int>(lane_word_scalar_codes.size(2));

    auto out =
        torch::empty({1, H, 1, T}, warp_code_major_factor_lut.options().dtype(torch::kFloat32));
    const dim3 block(THREADS, 1, 1);
    const dim3 grid((T + WARPS_PER_BLOCK - 1) / WARPS_PER_BLOCK, H, 1);

    turboquant_full_4bit_lane_word_factor_lut_warp_code_major_lane_nibble_qjl128_logits_b1q1_d128_kernel<<<grid, block>>>(
        warp_code_major_factor_lut.contiguous().data_ptr<float>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        ...
        &turboquant_full_4bit_lane_word_factor_lut_warp_code_major_lane_nibble_qjl128_logits_b1q1_d128_cuda,
        "Full TurboQuant factor LUT with warp-code-major [H,4,16,32] layout"
    );
}
