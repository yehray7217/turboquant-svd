...
/*
Combined-reduction factor-LUT variant.

Baseline factor-LUT full kernel:
  scalar_sum = warp_reduce(scalar_acc)
  qjl_sum    = warp_reduce(qjl_acc)
  out        = scalar_sum + scale * norm * qjl_sum

This variant:
  norm is loaded by lane 0 and broadcast to the warp.
  combined_acc = scalar_acc + scale * norm * qjl_acc
  out          = warp_reduce(combined_acc)

It removes one full warp reduction tree:
  baseline: 2 x (5 SHFL + 5 FADD)
  variant:  1 x (5 SHFL + 5 FADD)

The transformation is algebraically equivalent, but the FP summation order is
different, so tiny rounding differences vs the current factor-LUT kernel are expected.
*/
__global__ void turboquant_full_4bit_lane_word_factor_lut_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_kernel(
...
torch::Tensor turboquant_full_4bit_lane_word_factor_lut_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_cuda(
    ...
    turboquant_full_4bit_lane_word_factor_lut_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_kernel<<<grid, block>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        ...
        &turboquant_full_4bit_lane_word_factor_lut_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_cuda,
        "Full TurboQuant factor-LUT kernel with combined scalar/QJL warp reduction"
    );
}
