...
constexpr int FACTOR_LUT_ELEMS_PER_HEAD = D * SCALAR_LEVELS;  // 2048 floats
...
/*
Shared-staged factor-LUT combined-reduction kernel.

Current long-context best scalar path:
  global scalar_factor_lut[h, coord, code] scatter loads

Candidate:
  1. Each block handles one head h and 8 token warps.
  2. Cooperatively stage the full per-head FP32 factor LUT:
       128 * 16 = 2048 floats = 8 KB
     into shared memory.
  3. Each warp performs the same scalar indexed lookups from shared memory.

This directly tests whether the residual NCU bottleneck:
  factor-LUT indexed global loads / L1TEX pressure
can be traded for:
  one cooperative per-block global LUT read + shared-memory indexed lookups.
*/
__global__ void turboquant_full_4bit_lane_word_factor_lut_shared_stage_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_kernel(
    ...
    __shared__ float shared_factor_lut[FACTOR_LUT_ELEMS_PER_HEAD];
    ...
    const int64_t head_lut_base =
        static_cast<int64_t>(h) * FACTOR_LUT_ELEMS_PER_HEAD;

    #pragma unroll
    for (int i = tid; i < FACTOR_LUT_ELEMS_PER_HEAD; i += THREADS) {
        shared_factor_lut[i] = scalar_factor_lut[head_lut_base + i];
    }
    __syncthreads();
    ...
    for (int stage = 0; stage < 4; ++stage) {
        ...
        const int lut_idx = coord * SCALAR_LEVELS + scalar_code;
        scalar_acc += shared_factor_lut[lut_idx];
...
torch::Tensor turboquant_full_4bit_lane_word_factor_lut_shared_stage_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_cuda(
    ...
    turboquant_full_4bit_lane_word_factor_lut_shared_stage_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_kernel<<<grid, block>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        ...
        &turboquant_full_4bit_lane_word_factor_lut_shared_stage_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_cuda,
        "Full TurboQuant factor-LUT combined-reduction kernel with block-shared staged FP32 scalar LUT"
    );
}
