...
def pack_standard_qjl_signs(signs: torch.Tensor) -> torch.Tensor:
    if signs.shape[-1] != 128:
        raise ValueError("Expected signs last dim = 128.")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Full true TurboQuant optimized logits benchmark: baseline standard QJL signs "
            "vs lane-nibble signs + early residual norm load."
        )
    )
...
def main() -> None:
    ...
    print("========== Full true TurboQuant lane-nibble + early-norm benchmark ==========")
    print(f"device       = {device}")
    print(f"H,D,M        = {H},{D},{M}")
    print(f"seq_lens     = {list(args.seq_lens)}")
    print("[Compare] dense FP32, previous full baseline, optimized full lane-nibble+early-norm.")
    print("[Storage] QJL standard and lane-nibble signs both use 16 bytes/token/head.")
    ...
    for T in args.seq_lens:
        ...
        packed_scalar_codes = torch.randint(
        ...
        previous_full_ms, previous_full_out = bench_ms(
            lambda: turboquant_4bit_qjl128_logits_b1q1_d128_cuda(
                rotated_queries=rotated_queries,
                packed_scalar_codes=packed_scalar_codes,
        ...
        optimized_full_ms, optimized_full_out = bench_ms(
        ...
        parity = (optimized_full_out - previous_full_out).abs()

        result = {
            "seq_len": T,
            "timing_ms": {
                "dense_fp32_qkt_cuda_ms": float(dense_ms),
                "previous_full_turboquant_cuda_ms": float(previous_full_ms),
                "optimized_full_lane_nibble_early_norm_cuda_ms": float(optimized_full_ms),
            },
            "speedup_vs_dense": {
                "previous_full_over_dense": float(dense_ms / previous_full_ms),
                "optimized_full_over_dense": float(dense_ms / optimized_full_ms),
            },
            "speedup_vs_previous_full": {
                "optimized_full_over_previous_full": float(previous_full_ms / optimized_full_ms),
            },
            "parity_optimized_vs_previous_full": {
                "max_abs_diff": float(parity.max().item()),
                "mean_abs_diff": float(parity.mean().item()),
            },
            "sign_storage_bytes_per_token_head": {
                "previous_standard_packed_qjl_signs": 16,
                "lane_nibble_qjl_signs": 16,
            },
        }
        ...
        del rotated_queries, qjl_projected_queries, packed_scalar_codes
        del residual_norms, centroids, sign_values
        del standard_packed_signs, lane_nibble_signs
        del previous_full_out, optimized_full_out
        torch.cuda.empty_cache()

    payload = {
        "benchmark": "true_turboquant_full_lane_nibble_early_norm",
        "method": "previous_full_vs_lane_nibble_qjl_signs_plus_early_norm_full_kernel",
        "config": vars(args),
        "results": results,
    }
    out = Path(args.out)
