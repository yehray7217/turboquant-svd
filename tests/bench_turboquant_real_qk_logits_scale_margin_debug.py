...
DEFAULT_TEXT = (
    "TurboQuant logits-scale and margin diagnostic. "
    "This deterministic prompt is repeated to build a real language-model activation sequence. "
)
...
def tensor_stats(x: torch.Tensor) -> dict[str, float | int]:
    ...
    return {
        ...
        "std_finite_safe": float(safe.std(unbiased=False).item()),
    }


@torch.no_grad()
def basic_error(approx: torch.Tensor, target: torch.Tensor) -> dict[str, float]:
    approx = approx.to(torch.float32)
    target = target.to(torch.float32)
    diff = approx - target
    rmse = torch.sqrt(torch.mean(diff.square()))
    mae = diff.abs().mean()
    target_absmax = target.abs().max()
    target_std = target.std(unbiased=False)
    target_mean_abs = target.abs().mean()
    eps = 1e-12
    ...
    return {
        ...
        "target_absmax": float(target_absmax.item()),
        "target_std": float(target_std.item()),
        "target_mean_abs": float(target_mean_abs.item()),
        "rmse_over_absmax": float((rmse / target_absmax.clamp_min(eps)).item()),
        "rmse_over_std": float((rmse / target_std.clamp_min(eps)).item()),
        "rmse_over_mean_abs": float((rmse / target_mean_abs.clamp_min(eps)).item()),
    }


@torch.no_grad()
def topk_quality(approx: torch.Tensor, target: torch.Tensor, *, topk: int) -> dict[str, float]:
    approx = approx.to(torch.float32)
    target = target.to(torch.float32)
    ...
    target_top1 = torch.argmax(target, dim=-1)
    approx_top1 = torch.argmax(approx, dim=-1)
    top1_agreement = (target_top1 == approx_top1).float().mean()

    return {
        f"top{k}_overlap_vs_target": float(overlap.item()),
        "top1_agreement_vs_target": float(top1_agreement.item()),
...
@torch.no_grad()
def margin_diagnostics(approx: torch.Tensor, target: torch.Tensor) -> dict[str, float]:
    """
    Compute target top-1/top-2 margin and whether approximation error is large
    relative to that margin.
    Shapes: [B,H,Q,T]; Q is normally 1 here.
    """
    approx = approx.to(torch.float32)
    target = target.to(torch.float32)
    diff = approx - target

    target_top2_vals, target_top2_idx = torch.topk(target, k=2, dim=-1)
    target_top1_val = target_top2_vals[..., 0]
    target_top2_val = target_top2_vals[..., 1]
    target_margin = target_top1_val - target_top2_val

    target_top1_idx = target_top2_idx[..., 0]
    approx_top1_idx = torch.argmax(approx, dim=-1)
    top1_changed = target_top1_idx != approx_top1_idx

    # Approximation error evaluated at the target top-1 position.
    diff_abs = diff.abs()
    top1_abs_error = torch.gather(diff_abs, dim=-1, index=target_top1_idx.unsqueeze(-1)).squeeze(-1)

    # Global per-row maximum absolute logit error.
    row_max_abs_error = diff_abs.max(dim=-1).values

    eps = 1e-12
    top1_error_over_margin = top1_abs_error / target_margin.abs().clamp_min(eps)
    rowmax_error_over_margin = row_max_abs_error / target_margin.abs().clamp_min(eps)

    # Approximation's value at target top-1 vs its own top-1.
    approx_at_target_top1 = torch.gather(approx, dim=-1, index=target_top1_idx.unsqueeze(-1)).squeeze(-1)
    approx_own_top1_val = approx.max(dim=-1).values
    approx_top1_gap_against_target_top1 = approx_own_top1_val - approx_at_target_top1

    return {
        "target_top1_margin_mean": float(target_margin.mean().item()),
        "target_top1_margin_median": float(target_margin.median().item()),
        "target_top1_margin_min": float(target_margin.min().item()),
        "target_top1_margin_max": float(target_margin.max().item()),
        "target_top1_abs_error_mean": float(top1_abs_error.mean().item()),
        "target_top1_abs_error_median": float(top1_abs_error.median().item()),
        "row_max_abs_error_mean": float(row_max_abs_error.mean().item()),
        "row_max_abs_error_median": float(row_max_abs_error.median().item()),
        "top1_error_over_margin_mean": float(top1_error_over_margin.mean().item()),
        "top1_error_over_margin_median": float(top1_error_over_margin.median().item()),
        "rowmax_error_over_margin_mean": float(rowmax_error_over_margin.mean().item()),
        "rowmax_error_over_margin_median": float(rowmax_error_over_margin.median().item()),
        "fraction_target_top1_error_exceeds_margin": float((top1_abs_error >= target_margin.abs()).float().mean().item()),
        "fraction_rowmax_error_exceeds_margin": float((row_max_abs_error >= target_margin.abs()).float().mean().item()),
        "top1_changed_fraction": float(top1_changed.float().mean().item()),
        "approx_top1_gap_against_target_top1_mean": float(approx_top1_gap_against_target_top1.mean().item()),
        "approx_top1_gap_against_target_top1_median": float(approx_top1_gap_against_target_top1.median().item()),
    }
...
def fit_head_specific_codebooks(
    ...
    *,
    ...
    for h in range(H):
        ...
        codebooks.append(c)
    return torch.stack(codebooks, dim=0).contiguous()
...
def head_scalar_quantize(
    ...
    *,
    ...
    for h in range(H):
        x = rotated_keys[:, h, :, :].reshape(-1, D).to(torch.float32)
        c = head_centroids[h].to(device=rotated_keys.device, dtype=torch.float32).view(1, 1, -1)
        chunks = []
        for begin in range(0, int(x.shape[0]), chunk_rows):
            xx = x[begin : begin + chunk_rows]
            dist = torch.abs(xx.unsqueeze(-1) - c)
            chunks.append(torch.argmin(dist, dim=-1).to(torch.uint8))
        out[:, h, :, :] = torch.cat(chunks, dim=0).reshape(B, T, D)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Diagnose whether large raw RMSE is just logit scale, or large enough to break attention ranking."
    )
    ...
    p.add_argument(
        "--apply_attn_scale",
        action="store_true",
        help="Divide dense/TQ logits by sqrt(head_dim) before quality diagnostics.",
    )
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--rotation_seed", type=int, default=101)
    p.add_argument("--sketch_seed", type=int, default=202)
    p.add_argument("--sketch_type", choices=["gaussian", "rademacher"], default="gaussian")
...
def main() -> None:
    ...
    payload: dict[str, Any] = {
        "benchmark": "turboquant_real_qk_logits_scale_margin_debug",
        "config": {
            ...
            "apply_attn_scale": bool(args.apply_attn_scale),
        },
        "results": [],
    }

    print("========== TurboQuant logits-scale / margin diagnostic ==========")
    ...
    for layer_idx in args.layers:
        ...
        if str(args.sketch_type) == "rademacher":
            sketch = make_rademacher_sketch(
D,
            int(args.qjl_dim),
...
D,
        ...
        dense_stats = tensor_stats(dense)
        ...
        shared_logits = logits_from_encoding(
            qk.queries,
            rotation=rotation,
            sketch=sketch,
            encoding=shared_encoding,
        )
        head_logits = logits_from_encoding(
            qk.queries,
            rotation=rotation,
            sketch=sketch,
        ...
        def diagnostic_for(name: str, logits: dict[str, torch.Tensor]) -> dict[str, Any]:
            scalar = logits["scalar_logits"]
            full = logits["full_logits"]
            return {
                "scalar_only": {
                    "error": basic_error(scalar, dense),
                    "topk_quality": topk_quality(scalar, dense, topk=int(args.quality_topk)),
                    "margin": margin_diagnostics(scalar, dense),
                },
                "full_turboquant": {
                    "error": basic_error(full, dense),
                    "topk_quality": topk_quality(full, dense, topk=int(args.quality_topk)),
                    "margin": margin_diagnostics(full, dense),
                },
            }

        if bool(args.apply_attn_scale):
            scale = float(D) ** -0.5
            dense = dense * scale
            shared_logits = {
                k: v * scale for k, v in shared_logits.items()
            }
            head_logits = {
                k: v * scale for k, v in head_logits.items()
            }
            dense_stats = tensor_stats(dense)

        shared_diag = diagnostic_for("shared", shared_logits)
        head_diag = diagnostic_for("head_specific", head_logits)
        ...
        item = {
            ...
            "queries_stats": q_stats,
            "keys_stats": k_stats,
            "dense_logits_stats": dense_stats,
            "shared_codebook": shared_diag,
            "head_specific_codebook": head_diag,
        }
        print(json.dumps(item, indent=2))
        payload["results"].append(item)
