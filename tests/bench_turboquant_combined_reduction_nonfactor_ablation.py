...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Ablate whether factor LUT remains worthwhile after combined reduction."
        )
    )
...
def main() -> None:
    ...
    print("========== True TurboQuant combined-reduction non-factor ablation ==========")
    ...
    print("[Compare]")
    print("  prior non-factor best: separate reductions")
    print("  non-factor combined reduction")
    print("  factor-LUT combined reduction")
    print("[Purpose]")
    print("  Decide whether factor LUT remains worthwhile once the reduction tree is already optimized.")
    ...
    for T in args.seq_lens:
        ...
        combined_nonfactor_ms, combined_nonfactor_out = bench_ms(
        ...
        combined_factor_ms, combined_factor_out = bench_ms(
        ...
        combined_factor_effective_ms = lut_build_ms + combined_factor_ms

        diff_nonfactor = (combined_nonfactor_out - prior_nonfactor_out).abs()
        diff_factor_vs_nonfactor = (combined_factor_out - combined_nonfactor_out).abs()

        result = {
            "seq_len": T,
            "timing_ms": {
                "factor_lut_build_ms": float(lut_build_ms),
                "prior_nonfactor_best_separate_reduction_ms": float(prior_nonfactor_ms),
                "combined_reduction_nonfactor_ms": float(combined_nonfactor_ms),
                "combined_reduction_factor_lut_kernel_ms": float(combined_factor_ms),
                "combined_reduction_factor_lut_effective_build_plus_kernel_ms": float(
                    combined_factor_effective_ms
                ),
            },
            "speedup": {
                "combined_nonfactor_over_prior_nonfactor_best": float(
                    prior_nonfactor_ms / combined_nonfactor_ms
                ),
                "combined_factor_kernel_over_combined_nonfactor": float(
                    combined_nonfactor_ms / combined_factor_ms
                ),
                "combined_factor_effective_over_combined_nonfactor": float(
                    combined_nonfactor_ms / combined_factor_effective_ms
                ),
            },
            "parity": {
                "combined_nonfactor_vs_prior_nonfactor_best": {
                    "max_abs_diff": float(diff_nonfactor.max().item()),
                    "mean_abs_diff": float(diff_nonfactor.mean().item()),
                },
                "combined_factor_vs_combined_nonfactor": {
                    "max_abs_diff": float(diff_factor_vs_nonfactor.max().item()),
                    "mean_abs_diff": float(diff_factor_vs_nonfactor.mean().item()),
                },
            },
        }
        ...
        del prior_nonfactor_out, combined_nonfactor_out, combined_factor_out
        torch.cuda.empty_cache()

    payload = {
        "benchmark": "true_turboquant_combined_reduction_nonfactor_ablation",
        "method": (
            "prior_nonfactor_separate_vs_nonfactor_combined_vs_factor_lut_combined"
        ),
        "config": vars(args),
        "results": results,
    }
