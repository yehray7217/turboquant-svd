...
from turboquant.turboquant_factor_lut_block_geometry_ablation_cuda import (
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Factor-LUT combined block geometry ablation: "
            "production current vs 4/8/16 warps per block."
        )
    )
...
def main() -> None:
    ...
    print("========== True TurboQuant factor-LUT block geometry ablation ==========")
    ...
    print("[Compare]")
    print("  production_current: existing factor-LUT combined kernel")
    print("  geometry_wpb4:       4 warps/block, 128 threads")
    print("  geometry_wpb8:       8 warps/block, 256 threads")
    print("  geometry_wpb16:      16 warps/block, 512 threads")
    print("[Note]")
    print("  The scalar/QJL math is unchanged; only block geometry changes.")
    ...
    for T in args.seq_lens:
        ...
        wpb4_ms, wpb4_out = bench_ms(
            lambda: turboquant_factor_lut_combined_geometry_wpb4_cuda(
                scalar_factor_lut=scalar_factor_lut,
                lane_word_scalar_codes=lane_word_scalar_codes,
                qjl_projected_queries=qjl_projected_queries,
                lane_nibble_qjl_signs=lane_nibble_signs,
        ...
        wpb8_ms, wpb8_out = bench_ms(
            lambda: turboquant_factor_lut_combined_geometry_wpb8_cuda(
                scalar_factor_lut=scalar_factor_lut,
                lane_word_scalar_codes=lane_word_scalar_codes,
                qjl_projected_queries=qjl_projected_queries,
                lane_nibble_qjl_signs=lane_nibble_signs,
        ...
        wpb16_ms, wpb16_out = bench_ms(
            lambda: turboquant_factor_lut_combined_geometry_wpb16_cuda(
                scalar_factor_lut=scalar_factor_lut,
                lane_word_scalar_codes=lane_word_scalar_codes,
                qjl_projected_queries=qjl_projected_queries,
                lane_nibble_qjl_signs=lane_nibble_signs,
        ...
        variants = {
            "production_current": (prod_ms, prod_out),
            "geometry_wpb4": (wpb4_ms, wpb4_out),
            "geometry_wpb8": (wpb8_ms, wpb8_out),
            "geometry_wpb16": (wpb16_ms, wpb16_out),
        }
        best_name, (best_ms, _) = min(variants.items(), key=lambda kv: kv[1][0])

        parity = {}
        for name, (_, out) in variants.items():
            diff = (out - prod_out).abs()
            parity[name] = {
        ...
        del prod_out, wpb4_out, wpb8_out, wpb16_out
        torch.cuda.empty_cache()

    payload = {
        "benchmark": "true_turboquant_factor_lut_block_geometry_ablation",
        "method": "production_vs_wpb4_vs_wpb8_vs_wpb16",
        "config": vars(args),
        "results": results,
    }
