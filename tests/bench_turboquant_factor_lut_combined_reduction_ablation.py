...
from turboquant.turboquant_factor_lut_combined_reduction_ablation_cuda import (
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "True TurboQuant factor-LUT combined-reduction ablation: "
            "current factor-LUT full vs one-reduction variant."
        )
    )
...
def main() -> None:
    ...
    print("========== True TurboQuant factor-LUT combined reduction ablation ==========")
    ...
    print("[Compare]")
    print("  current factor-LUT full: separate scalar and QJL warp reductions")
    print("  combined-reduction full: scale per lane, then one warp reduction")
    print("[Note]")
    print("  Algebraically equivalent; tiny FP-rounding differences are expected.")
    ...
    for T in args.seq_lens:
        ...
        combined_ms, combined_out = bench_ms(
            lambda: turboquant_full_4bit_lane_word_factor_lut_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_cuda(
                scalar_factor_lut=scalar_factor_lut,
                lane_word_scalar_codes=lane_word_scalar_codes,
                qjl_projected_queries=qjl_projected_queries,
                lane_nibble_qjl_signs=lane_nibble_signs,
        ...
        parity = (combined_out - current_out).abs()
        current_effective_ms = build_ms + current_ms
        combined_effective_ms = build_ms + combined_ms
        ...
        result = {
            ...
            "timing_ms": {
                ...
                "combined_reduction_factor_lut_full_kernel_ms": float(combined_ms),
                "current_factor_lut_effective_build_plus_kernel_ms": float(current_effective_ms),
                "combined_reduction_factor_lut_effective_build_plus_kernel_ms": float(combined_effective_ms),
            },
            "speedup": {
                "combined_reduction_kernel_over_current_factor_lut_kernel": float(current_ms / combined_ms),
                "combined_reduction_effective_over_current_factor_lut_effective": float(
                    current_effective_ms / combined_effective_ms
                ),
            },
            "parity_combined_reduction_vs_current_factor_lut": {
                "max_abs_diff": float(parity.max().item()),
                "mean_abs_diff": float(parity.mean().item()),
            },
            "reduction_count_per_warp": {
                "current_factor_lut_full": 2,
                "combined_reduction_full": 1,
            },
        }
        ...
        del scalar_factor_lut, current_out, combined_out
        torch.cuda.empty_cache()

    payload = {
        "benchmark": "true_turboquant_factor_lut_combined_reduction_ablation",
        "method": "current_factor_lut_full_vs_combined_scalar_qjl_reduction_full",
        "config": vars(args),
        "results": results,
    }
    out = Path(args.out)
