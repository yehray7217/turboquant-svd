...
@torch.no_grad()
def bench_scalar_factor_lut_build_ms(
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Consolidated true TurboQuant factor-LUT benchmark: "
            "dense FP32 qK^T vs current best full vs factor-LUT full, "
            "plus factor-LUT build overhead."
        )
    )
...
def main() -> None:
    ...
    print("========== Consolidated true TurboQuant factor-LUT benchmark ==========")
    ...
    print("  current best full: scalar lane-word + QJL lane-nibble")
    print("  factor-LUT full: scalar factor LUT + QJL lane-nibble")
    print("[Overhead]")
    print("  report scalar factor-LUT build time separately")
    print("  report factor-LUT full effective time = LUT build + kernel")
    print("[Factor LUT]")
    print("  shape = [1,H,128,16], FP32")
    print("  storage = 8192 bytes/head")
    ...
    for T in args.seq_lens:
        ...
        factor_build_ms, scalar_factor_lut = bench_scalar_factor_lut_build_ms(
            rotated_queries,
            centroids,
        ...
        factor_effective_ms = factor_build_ms + factor_ms
        parity = (factor_out - current_out).abs()

        result = {
            "seq_len": T,
            "timing_ms": {
                "dense_fp32_qkt_cuda_ms": float(dense_ms),
                "current_best_full_cuda_ms": float(current_ms),
                "factor_lut_build_ms": float(factor_build_ms),
                "factor_lut_full_kernel_ms": float(factor_ms),
                "factor_lut_full_effective_build_plus_kernel_ms": float(factor_effective_ms),
            },
            "speedup_vs_dense": {
                "current_best_full_over_dense": float(dense_ms / current_ms),
                "factor_lut_kernel_only_full_over_dense": float(dense_ms / factor_ms),
                "factor_lut_effective_build_plus_kernel_over_dense": float(
                    dense_ms / factor_effective_ms
                ),
            },
            "speedup_vs_current_best_full": {
                "factor_lut_kernel_only_over_current_best": float(current_ms / factor_ms),
                "factor_lut_effective_build_plus_kernel_over_current_best": float(
                    current_ms / factor_effective_ms
                ),
            },
            "parity_factor_lut_vs_current_best_full": {
        ...
        del scalar_factor_lut, current_out, factor_out
        torch.cuda.empty_cache()

    payload = {
        "benchmark": "true_turboquant_factor_lut_consolidated",
        "method": (
            "dense_fp32_vs_current_best_full_vs_factor_lut_full_"
            "with_factor_lut_build_overhead"
        ),
        "config": vars(args),
        "results": results,
    }
