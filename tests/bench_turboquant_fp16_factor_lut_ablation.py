...
from turboquant.fp16_factor_lut import build_scalar_factor_lut_fp16
...
from turboquant.turboquant_fp16_factor_lut_ablation_cuda import (
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "FP16 scalar factor-LUT ablation: FP32 factor-LUT combined kernel "
            "vs FP16 factor-LUT combined kernel."
        )
    )
...
def main() -> None:
    ...
    print("========== True TurboQuant FP16 factor-LUT ablation ==========")
    ...
    print("[Compare]")
    print("  FP32 scalar factor LUT + combined reduction")
    print("  FP16 scalar factor LUT + combined reduction")
    print("[Storage]")
    print("  FP32 factor LUT: 8192 bytes/head")
    print("  FP16 factor LUT: 4096 bytes/head")
    print("[Numerics]")
    print("  Accumulation remains FP32; only scalar factor LUT storage is FP16.")
    ...
    for T in args.seq_lens:
        ...
        fp32_build_ms, scalar_factor_lut_fp32 = bench_ms(
            lambda: build_scalar_factor_lut_fp32(rotated_queries, centroids),
            warmup=args.build_warmup,
            iters=args.build_iters,
        )
        fp16_build_ms, scalar_factor_lut_fp16 = bench_ms(
            lambda: build_scalar_factor_lut_fp16(rotated_queries, centroids),
            warmup=args.build_warmup,
            iters=args.build_iters,
        )

        fp32_ms, fp32_out = bench_ms(
            lambda: turboquant_factor_lut_combined_reduction_4bit_qjl128_logits_b1q1_d128_cuda(
                scalar_factor_lut=scalar_factor_lut_fp32,
        ...
        fp16_ms, fp16_out = bench_ms(
            lambda: turboquant_full_4bit_lane_word_factor_lut_fp16_lane_nibble_qjl128_combined_reduction_logits_b1q1_d128_cuda(
                scalar_factor_lut_fp16=scalar_factor_lut_fp16,
                lane_word_scalar_codes=lane_word_scalar_codes,
                qjl_projected_queries=qjl_projected_queries,
                lane_nibble_qjl_signs=lane_nibble_signs,
                residual_norms=residual_norms,
        ...
        diff = (fp16_out - fp32_out).abs()
        fp32_effective_ms = fp32_build_ms + fp32_ms
        fp16_effective_ms = fp16_build_ms + fp16_ms

        result = {
            "seq_len": T,
            "timing_ms": {
                "fp32_factor_lut_build_ms": float(fp32_build_ms),
                "fp16_factor_lut_build_ms": float(fp16_build_ms),
                "fp32_factor_lut_combined_kernel_ms": float(fp32_ms),
                "fp16_factor_lut_combined_kernel_ms": float(fp16_ms),
                "fp32_factor_lut_effective_build_plus_kernel_ms": float(fp32_effective_ms),
                "fp16_factor_lut_effective_build_plus_kernel_ms": float(fp16_effective_ms),
            },
            "speedup": {
                "fp16_kernel_over_fp32_kernel": float(fp32_ms / fp16_ms),
                "fp16_effective_over_fp32_effective": float(fp32_effective_ms / fp16_effective_ms),
            },
            "error_fp16_vs_fp32_factor_lut": {
                "max_abs_diff": float(diff.max().item()),
                "mean_abs_diff": float(diff.mean().item()),
                "rmse": float(torch.sqrt((diff * diff).mean()).item()),
            },
            "factor_lut_storage_bytes_per_head": {
                "fp32": 8192,
                "fp16": 4096,
            },
        }
        ...
        del scalar_factor_lut_fp32, scalar_factor_lut_fp16, fp32_out, fp16_out
        torch.cuda.empty_cache()

    payload = {
        "benchmark": "true_turboquant_fp16_factor_lut_ablation",
        "method": "fp32_factor_lut_combined_vs_fp16_factor_lut_combined",
        "config": vars(args),
        "results": results,
    }
