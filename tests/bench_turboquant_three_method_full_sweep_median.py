...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Robust median synthetic full sweep for dense FP32, non-factor combined, "
...
def infer_piecewise_policy(
    results: list[dict[str, Any]],
    *,
    hysteresis_ratio: float = 0.01,
) -> dict[str, Any]:
    """
    Produce a cautious policy suggestion from medians.

    A factor-LUT "win" over nonfactor is only counted when it is faster by more
    than hysteresis_ratio. This avoids toggling due to near-ties.
    """
    dense_to_nonfactor_first_t = None
    factor_over_nonfactor_first_t = None
    factor_stable_suffix_t = None

    for r in results:
        t = int(r["seq_len"])
        d = r["timing_summary_ms"]
        dense = float(d["dense_fp32_qkt_cuda"]["median_ms"])
        nf = float(d["nonfactor_combined_cuda"]["median_ms"])
        factor_eff = float(
            d["factor_lut_combined_effective_global_build_plus_kernel"]["median_ms"]
        )

        if dense_to_nonfactor_first_t is None and nf < dense:
            dense_to_nonfactor_first_t = t

        if factor_over_nonfactor_first_t is None and factor_eff < nf * (1.0 - hysteresis_ratio):
            factor_over_nonfactor_first_t = t

    # Stable suffix: first T such that all later points are factor wins over nonfactor by hysteresis.
    for i, r in enumerate(results):
        suffix = results[i:]
        if all(
            float(x["timing_summary_ms"]["factor_lut_combined_effective_global_build_plus_kernel"]["median_ms"])
            < float(x["timing_summary_ms"]["nonfactor_combined_cuda"]["median_ms"]) * (1.0 - hysteresis_ratio)
            for x in suffix
        ):
            factor_stable_suffix_t = int(r["seq_len"])
            break

    return {
        "hysteresis_ratio": float(hysteresis_ratio),
        "dense_to_nonfactor_first_t": dense_to_nonfactor_first_t,
        "factor_effective_over_nonfactor_first_t": factor_over_nonfactor_first_t,
        "factor_effective_stable_suffix_t": factor_stable_suffix_t,
    }
...
def main() -> None:
    ...
    print("========== True TurboQuant three-method full sweep, robust median ==========")
    ...
    print("[Packed synthetic input]")
    print("  lane_word_scalar_codes and lane_nibble_qjl_signs are generated directly as packed uint8 payloads.")
    print("-" * 196)
    print(
    ...
    for T in seq_lens:
        ...
        # Make tensors deterministic per T; only measurement noise should vary across repeats.
    ...
    policy_inference = infer_piecewise_policy(results, hysteresis_ratio=0.01)
    ...
    payload = {
        "benchmark": "true_turboquant_three_method_full_sweep_median",
        "method": (
            "synthetic_1k_step_robust_median_sweep_dense_vs_nonfactor_vs_factor_lut_"
